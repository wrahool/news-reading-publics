...
#build master networks for 18 months only Jan 2017 to Jun 2018.
...
only_18_months = c("January2017", "February2017", "March2017", "April2017", "May2017", "June2017", "July2017", "August2017", "September2017",
                   "October2017", "November2017", "December2017", "January2018", "February2018", "March2018", "April2018", "May2018", "June2018")

all_months = read.csv("03_Auxiliary/months.csv", as.is = T)

reqd_18_indices =  which(all_months$months %in% only_18_months)

red_graphs_list2 = vector("list", length(reqd_18_indices))

j = 1
for(i in reqd_18_indices) {
  reqd.node.ids = unlist(lapply(reqd.media, FUN = function(x) which(x == V(red_graphs_list[[i]])$name)))
  red_graphs_list2[[j]] = induced.subgraph(graph = red_graphs_list[[i]], vids = reqd.node.ids)
  j=j+1
}
...
KM.master.df = KM.master.df[KM.master.df$Month %in% only_18_months,]
...
TI.df = TI.df[TI.df$Month %in% only_18_months,]
...
save(master.g, final.master.g, filtered.master.g, file = "04_RData/04_master_networks_with_18months.RData")
