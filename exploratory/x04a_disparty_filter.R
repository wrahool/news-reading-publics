...
#first, let's remove the insignificant ties (using disparity filter)
...
dp_filtered_graphs_list = vector("list", length(red_graphs_list))
central.df = NULL
for(i in 1:length(red_graphs_list2)){
  ...
  E(g)$weight = E(g)$shared_audience
  dp1_fg = disparity_filter(g, alpha = 0.01)
  dp2_fg = getBackboneNetwork(g, alpha = 0.01, max.vertices = 177, use.original.alpha = FALSE)
  
  gx = g
  V(gx)$name = NULL
  dp3_g = disparityfilter::backbone(gx, weights = E(gx)$weight, directed = F, alpha = 0.01)
...
save(dp_filtered_graphs_list, file = "04_RData/03_filtered_networks.RData")
