...
# test for one value of Type
all_vernacular_to_national_tbl %>%
  filter(Type == "Digital-born") %>%
  pull(MeanPO) %>%
  ts(frequency = 12, start = c(2014, 10)) %>%
  sens.slope()
...
# check slope for each type 
all_vernacular_to_international_tbl %>%
  filter(Type == "Digital-born") %>%
  pull(MeanPO) %>%
  ts(frequency = 12, start = c(2014, 10)) %>%
  sens.slope()
